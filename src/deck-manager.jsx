...
async function scanLineupScreen(base64, mediaType, role, userId) {
  ...
  var prompt = [
    ...
    '- YES (숫자가 있다) → 2단계로 이동.',
    '',
    '[2단계: 시그니처 판정 - 임팩트 다음으로 최우선. 골든글러브보다 반드시 먼저 확인]',
    '★★★ 이 단계는 3단계(골든글러브)보다 절대적으로 우선한다. 순서를 바꾸지 말 것. ★★★',
    '- 지금 당장 카드에서 선수 이름 텍스트를 찾아라.',
    '- 그 이름의 왼쪽 위 방향을 정밀하게 살펴라.',
    '- 찾을 것: 빨간색 필기체로 쓴 기울어진 "S" 또는 "Sig" 형태의 작은 로고.',
    '- 이 로고의 특징: 손으로 서명하듯 흘려 쓴 빨간 글자. 크기는 작지만 색상이 선명한 빨간색.',
    '- 이 로고는 이름 글자 왼쪽 위에 바짝 붙어 있다. 카드 전체가 아닌 이름 주변만 집중 확인.',
    '- 배경이 황금색이어도, 분홍색이어도 상관없다. 빨간 필기체 S/Sig 로고가 있으면 무조건 시그니처.',
    '- YES (빨간 필기체 로고 확인됨) → 즉시 시그니처 확정. 골든글러브 검사 건너뜀.',
    '- NO (어디에도 없음) → 3단계로 이동.',
    '',
    '[3단계: 골든글러브 판정]',
    ...
    '- 이름 왼쪽 위의 빨간 필기체 S/Sig 로고 → 100% 시그니처 (배경색·테두리 무관, 임팩트 다음 최우선 확인)',
